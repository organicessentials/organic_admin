...
import { index, destroy } from '../../service/api/category';
...
const List =() => {
    ...
    const deleteAction = () => {
        if(record){
            destroy(record).then(data => getData())
    ...
    const rightToolbarTemplate = () => {
        return (
            <React.Fragment>
                <RedirectButton route="/category/add" label="Add Category" icon="pi pi-plus" severity="sucess" className="mr-2"  />
            </React.Fragment>
        );
    };
...
List.getInitialProps = async () => {
    ...
    return { pageTitle:{url:'/category',name:"Category Page"} };
};
