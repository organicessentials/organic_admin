...
import { index, destroy } from '../../service/api/post';
...
const List =() => {
    ...
    const rightToolbarTemplate = () => {
        return (
            <React.Fragment>
                <RedirectButton route="/post/add" label="Add Post" icon="pi pi-plus" severity="sucess" className="mr-2"  />
            </React.Fragment>
        );
    };
    ...
    const actionBodyTemplate = (rowData) => {
        return (
            <React.Fragment>
                <RedirectButton route={"/post/"+rowData._id} icon="pi pi-pencil" rounded outlined className="mr-2" />
...
List.getInitialProps = async () => {
    ...
    return { pageTitle:{url:'/post',name:"Post"} };
};
